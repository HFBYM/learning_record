...
//#include<cctype>              //字符处理函数 isalpha isdigital之类的
...
std::wstring G2U(const std::string& str)
{
	...
	memset(wstr, 0, len);
	MultiByteToWideChar(CP_ACP, 0, str.c_str(), -1, wstr, len); //len是缓冲区大小 包括NULL 以字符为单位
...
int main()
{
	...
	ifs.imbue(std::locale("zh_CN.UTF-8")); //文件字符为UTF8
	ifs.open("input.txt", std::ios::in);
	if (!ifs.is_open())
		__debugbreak();  //判断文件是否存在

	std::wofstream ofs;
	ofs.imbue(std::locale("zh_CN.UTF-8")); //文件字符为UTF8
